...
class ImageToVideoTaskProcessor(QThread):
    ...
    def _process_model(self, ai_clothes):
        ...
        task = self.local_db_manager.get_image_to_video_gen_task_by_id(self.local_task_id)
    ...
    def _process_video(self, dressed_model):
        task = self.local_db_manager.get_image_to_video_gen_task_by_id(self.local_task_id)
        video_gen_config_dict = self.local_db_manager.get_image_to_video_gen_config_by_id(task['image_to_video_gen_config_id'])
...
class ImageToVideoGenDialog(QDialog):
    def __init__(self, parent=None, db_manager=None):
        super().__init__(parent)
        self.db_manager = db_manager
        self.setWindowTitle('创建图生视频生成任务')
    ...
    def init_ui(self):
        ...
        layout.addWidget(QLabel('选择图生视频配置:'))
    ...
    def on_ok(self):
        ...
        if video_config_index == 0:
            QMessageBox.warning(self, '警告', '请选择图生视频配置')
            return
        ...
        self.local_task_id = self.db_manager.add_image_to_video_gen_task(
            ...
            image_to_video_gen_config_id=selected_video_config['id'],
            status=ImageToVideoGenTask.STATUS_SUBMITTED
        )
        ...
        self.task_processor = ImageToVideoTaskProcessor(task_data)
