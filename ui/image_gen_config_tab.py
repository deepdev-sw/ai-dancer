...
class ImageGenConfigTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.type_combo.addItem(ImageGenConfig.CONFIG_TYPE_VOLC_ENGINE)
    ...
    def add_config(self):
        ...
        dialog = ConfigEditDialog(self, name=name, config_type=config_type)
        if dialog.exec_() == QDialog.Accepted:
            try:
                self.db_manager.add_image_gen_config(name, config_type, dialog.get_config_content())
    ...
    def edit_config(self, config_id, name, config_type, config_content):
        dialog = ConfigEditDialog(self, name=name, config_type=config_type, config_content=config_content)
        if dialog.exec_() == QDialog.Accepted:
            try:
                self.db_manager.update_image_gen_config(config_id, dialog.name_input.text().strip(), 
    ...
    def delete_config(self, config_id):
        ...
        if reply == QMessageBox.Yes:
            try:
                self.db_manager.delete_image_gen_config(config_id)
                self.load_data()
...
class ConfigEditDialog(QDialog):
    def __init__(self, parent=None, name='', config_type='', config_content=''):
        ...
        self.model_input.setPlaceholderText('模型ID')
        ...
        self.size_input.setPlaceholderText('图像尺寸（如：2K）')
        layout.addWidget(QLabel('图像尺寸:'))
        layout.addWidget(self.size_input)
        
        self.output_format_input = QLineEdit()
        self.output_format_input.setPlaceholderText('输出图像格式（如：jpeg）')
        layout.addWidget(QLabel('输出图像格式:'))
        layout.addWidget(self.output_format_input)
        ...
        if config_content:
            try:
                ...
                self.output_format_input.setText(data.get('output_format', ''))
            except:
                pass
        else:
            self.model_input.setText('doubao-seedream-5-0-260128')
            self.size_input.setText('2K')
            self.output_format_input.setText('jpeg')
